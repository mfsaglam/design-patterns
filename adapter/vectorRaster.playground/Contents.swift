...
class VectorRectangle: VectorObject {
    init(_ x: Int, _ y: Int, _ width: Int, _ height: Int) {
        super.init()
        lines.append(Line(Point(x, y), Point(x+width, y+height)))
...
class LineToPointAdapter: Sequence {
    ...
    init(_ line: Line) {
        ...
        let left = Swift.min(line.start.x, line.start.y)
        let right = Swift.max(line.start.x, line.start.y)
        let top = Swift.min(line.end.x, line.end.y)
        let bottom = Swift.max(line.end.x, line.end.y)
